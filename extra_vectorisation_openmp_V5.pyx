...
"""
def main(time_power, int threads):    
    time_step = 60
    times = []
    bodies=[]
    ...
    for asteroid_frequency in np.arange(0,5):
        ...
        time_start = openmp.omp_get_wtime()
        ...
        """The grav an suvat functions have been moved in here and altered so 
        that it is compatible with the release of thr GIL
        """
        for t in np.arange(60,3.154 * 10 **(5),time_step):         
            ...
            for i in np.arange(0,len(full_array)):
                ...
                with nogil:
                    for l in prange(0,length, num_threads=threads):
                        with gil:
                            mj = full_array[l,0]
                ...
                acceleration =[ax,ay]    
                new_position= full_array[i,[2,3]]+full_array[i,[4,5]]*time_step + 0.5*np.array(acceleration)*time_step**2
                new_velocity=full_array[i,[4,5]] + np.array(acceleration)*time_step
                          
            
                full_array[:,[2,3]] = new_position
                full_array[:,[4,5]] = new_velocity
                
                
        time_end = openmp.omp_get_wtime()
